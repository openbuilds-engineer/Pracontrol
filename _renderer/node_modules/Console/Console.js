...
module.exports = class Console extends WebComponentAbstract {
  init() {
    ...
    this.console = this.new('console')
    
    if(!('commandHistory' in this.ser)) this.ser.commandHistory = []
    this.commandHistoryIndex = this.ser.commandHistory.length
    
    this.input = this.new('input', { placeholder: 'Send command' })
    this.input.on('keypress', e => e.keyCode == 13 && this.submitInput())
    
    // focus tab when focus to input
    this.input.on('focus', e => { self.emit('focusTab', this); if(!e.isTrusted) this.input.focus(); })
    
    defineShortcut('⌘Enter', 'Focus console input', 'Console')
    self.on('keydown', e => (e.metaKey || e.ctrlKey) && e.keyCode == 13 && self.emit('focusTab', this))
    
    defineShortcut('Up/Down', 'Commands history', 'Console')
    this.input.on('keydown', e => this.historyCommand(e))
    
    defineEvent('serialEcho', 'Device sends echo message, listen only', 'Console', `''`)
    defineEvent('serialOk', 'Device sends ok message, listen only', 'Console', `''`)
    self.on('serialData', e => this.serialData(e.detail.data))
    this.firstPart = ''
    
    defineEvent('consoleClear', 'Clear console', 'Console')
    self.on('consoleClear', e => this.consoleClear())
    self.on('connected', e => this.consoleClear())
      
    defineShortcut('⌘K', 'Clear console', 'Console')
    self.on('keydown', e => (e.metaKey || e.ctrlKey) && e.keyCode == 75 && this.consoleClear())
    
    defineEvent('consoleInputValue', 'Set console input value and focus it', 'Console', `''`)
    self.on('consoleInputValue', e => { this.input.value = e.detail; self.emit('focusTab', this) })
    
    // scroll down on tab focus
    this.on('tabFocus', e => this.scrollDown())
    
    self.emit('newTab', { el: this, name: 'Console', priority: -80 })
  }
  ...
  submitInput() {
    ...
    var e = self.emit('serialWrite', { data: this.input.value, prepend: true })
    this.input.value = ''
