module.exports = class ControlShortcuts extends WebComponentAbstract {
  init() {
    ...
    self.on('deviceReady', e => AppEvent('serialWrite', { data: 'G1 F100000' }) )
    ...
    defineShortcut('Esc', 'Abort moves and clean buffer', 'gCode')
    defineShortcut('§', 'Emergency stop', 'gCode')
  }
  
  keyShortcut(e) {
    if(e.keyCode == 27 || e.keyCode == 192) {
      AppEvent('serialDirectWrite', { data: e.keyCode == 192 ? 'M112\n' : 'M410\n' })
  ...
  move(axis, vel) {
    AppEvent('serialWrite', { data: ['G91', `G1 ${axis} ${vel} F100000`, 'G90', 'M400'], prepend: true })
  }
}
