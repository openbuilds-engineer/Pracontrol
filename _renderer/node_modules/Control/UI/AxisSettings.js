...
module.exports = class AxisSettings extends WebComponentAbstract {
  ...
  readyCallback() {
    ...
    var p = this.new('p', true, { textContent: this.opt.name })
    p.new('br')
    
    var opt = { type: 'number', step: 1 }
    this.X = p.new('label', true, { textContent: 'X '}).new('input', true, opt)
    this.X.on('input', e => this.set())
    p.newText(' ')
    
    this.Y = p.new('label', true, { textContent: 'Y '}).new('input', true, opt)
    this.Y.on('input', e => this.set())
    p.new('br')
    
    this.Z = p.new('label', true, { textContent: 'Z '}).new('input', true, opt)
    this.Z.on('input', e => this.set())
    p.newText(' ')
    
    this.E = p.new('label', true, { textContent: 'E '}).new('input', true, opt)
  ...
  set() {
    ...
    self.emit('serialWrite', { data: `${this.opt.gCode} ${val}` })
  }
