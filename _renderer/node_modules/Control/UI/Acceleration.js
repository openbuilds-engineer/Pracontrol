...
module.exports = class Acceleration extends WebComponentAbstract {
  init() {
    ...
    var p = this.new('p', true, { textContent: 'Max acceleration ' })
    p.new('small', true, { textContent: 'mm/sec^2' })
    p.new('br')
    var opt = { type: 'number', step: 1 }
    var l = p.new('label')
    l.new('span', true, { textContent: 'Travel ', className: 'acc' })
    this.accTravel = l.new('input', true, opt)
    this.accTravel.on('input', e => this.accSet())
    p.new('br')
    var l = p.new('label')
    l.new('span', true, { textContent: 'Print ', className: 'acc' })
    this.accPrint = l.new('input', true, opt)
    this.accPrint.on('input', e => this.accSet())
    p.new('br')
    var l = p.new('label')
    l.new('span', true, { textContent: 'Retract ', className: 'acc' })
    this.accRetract = l.new('input', true, opt)
    this.accRetract.on('input', e => this.accSet())
    
    
    var p = this.new('p', true, { textContent: 'Max jerk ' })
    p.new('small', true, { textContent: 'mm/s/s' })
    p.new('br')
    var opt = { type: 'number', step: 1, className: 'jerk' }
    this.jerkXY = p.new('label', true, { textContent: 'XY ' }).new('input', true, opt)
    this.jerkXY.on('input', e => this.advancedSet())
    p.newText(' ')
    this.jerkZ = p.new('label', true, { textContent: 'Z ' }).new('input', true, opt)
    this.jerkZ.on('input', e => this.advancedSet())
    p.newText(' ')
    this.jerkE = p.new('label', true, { textContent: 'E ' }).new('input', true, opt)
    this.jerkE.on('input', e => this.advancedSet())
    p.newText(' ')
    
    var p = this.new('p', true, { textContent: 'Min feed rate ' })
    p.new('small', true, { textContent: 'mm/s/s' })
    p.new('br')
    var opt = { type: 'number', step: 1 }
    this.minFeedTravel = p.new('label', true, { textContent: 'Travel '}).new('input', true, opt)
    this.minFeedTravel.on('input', e => this.advancedSet())
    p.newText(' ')
    this.minFeedPrint = p.new('label', true, { textContent: 'Print '}).new('input', true, opt)
    this.minFeedPrint.on('input', e => this.advancedSet())
    
    var p = this.new('p', true, { textContent: 'Min segment time ' })
    var opt = { type: 'number', step: 1 }
    this.segmentTime = p.new('input', true, opt)
    this.segmentTime.on('input', e => this.advancedSet())
    p.newText(' ')
    p.new('small', true, { textContent: 'µs' })
  ...
  accSet() {
    ...
    self.emit('serialWrite', { data: `M204 ${val}` })
  }
  ...
  advancedSet() {
    ...
    self.emit('serialWrite', { data: `M205 ${val}` })
  }
