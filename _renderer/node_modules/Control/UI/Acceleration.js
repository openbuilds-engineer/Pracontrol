...
module.exports = class Acceleration extends WebComponentAbstract {
  ...
  accSet() {
    var val = `P${this.accPrint.value} R${this.accRetract.value} V${this.accTravel.value}`
    self.emit('serialWrite', { data: `M204 ${val}`, prepend: true })
  }
  
  accValue(msg) {
    var p = msg.match(/P([0-9]+\.?[0-9]*)/)
    if(p) { this.accPrint.value = parseFloat(p[1]) }
    
    var r = msg.match(/R([0-9]+\.?[0-9]*)/)
    if(r) { this.accRetract.value = parseFloat(r[1]) }
    
    var v = msg.match(/V([0-9]+\.?[0-9]*)/)
    if(v) { this.accTravel.value = parseFloat(v[1]) }
  }
