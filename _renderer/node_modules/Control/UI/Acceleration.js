...
module.exports = class Acceleration extends WebComponentAbstract {
  initCallback() {
    ...
    var p = this.newElem('p', true, { textContent: 'Max acceleration ' })
    p.newElem('small', true, { textContent: 'mm/sec^2' })
    p.newElem('br')
    var opt = { type: 'number', step: 1 }
    var l = p.newElem('label')
    l.newElem('span', true, { textContent: 'Travel ', className: 'acc' })
    this.accTravel = l.newElem('input', true, opt)
    this.accTravel.on('input', e => this.accSet())
    p.newElem('br')
    var l = p.newElem('label')
    l.newElem('span', true, { textContent: 'Print ', className: 'acc' })
    this.accPrint = l.newElem('input', true, opt)
    this.accPrint.on('input', e => this.accSet())
    p.newElem('br')
    var l = p.newElem('label')
    l.newElem('span', true, { textContent: 'Retract ', className: 'acc' })
    this.accRetract = l.newElem('input', true, opt)
    this.accRetract.on('input', e => this.accSet())
    
    
    var p = this.newElem('p', true, { textContent: 'Max jerk ' })
    p.newElem('small', true, { textContent: 'mm/s/s' })
    p.newElem('br')
    var opt = { type: 'number', step: 1, className: 'jerk' }
    this.jerkXY = p.newElem('label', true, { textContent: 'XY ' }).newElem('input', true, opt)
    this.jerkXY.on('input', e => this.advancedSet())
    p.newText(' ')
    this.jerkZ = p.newElem('label', true, { textContent: 'Z ' }).newElem('input', true, opt)
    this.jerkZ.on('input', e => this.advancedSet())
    p.newText(' ')
    this.jerkE = p.newElem('label', true, { textContent: 'E ' }).newElem('input', true, opt)
    this.jerkE.on('input', e => this.advancedSet())
    p.newText(' ')
    
    var p = this.newElem('p', true, { textContent: 'Min feed rate ' })
    p.newElem('small', true, { textContent: 'mm/s/s' })
    p.newElem('br')
    var opt = { type: 'number', step: 1 }
    this.minFeedTravel = p.newElem('label', true, { textContent: 'Travel '}).newElem('input', true, opt)
    this.minFeedTravel.on('input', e => this.advancedSet())
    p.newText(' ')
    this.minFeedPrint = p.newElem('label', true, { textContent: 'Print '}).newElem('input', true, opt)
    this.minFeedPrint.on('input', e => this.advancedSet())
    
    var p = this.newElem('p', true, { textContent: 'Min segment time ' })
    var opt = { type: 'number', step: 1 }
    this.segmentTime = p.newElem('input', true, opt)
    this.segmentTime.on('input', e => this.advancedSet())
    p.newText(' ')
    p.newElem('small', true, { textContent: 'µs' })
