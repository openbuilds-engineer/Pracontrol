module.exports = class PasteExtruder extends WebComponentAbstract {
  init() {
    var p = this.new('p', { textContent: 'Heater 1 ' })
    var slider1 = p.new('input', { type: 'range', min: 0, max: 255, step: 1, value: 0 })
    slider1.on('input', e => this.send(`M126 S${slider1.value}`))
    
    var p = this.new('p', { textContent: 'Heater 2 ' })
    var slider2 = p.new('input', { type: 'range', min: 0, max: 255, step: 1, value: 0 })
    slider2.on('input', e => this.send(`M128 S${slider2.value}`))
  }

  send(val) {
    self.emit('serialWrite', { data: val, prepend: true })
  }
}
