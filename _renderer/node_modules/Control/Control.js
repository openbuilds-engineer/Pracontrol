...
module.exports = class Control extends TabComponent {
  init() {
    ...
    defineEvent('newControl', 'Add item to Control', 'Control', "{ tab: '', cat: '', gcode: '', name: '' or el: <HTMLElement>, tab: '' }")
    ...
    setImmediate(() => self.emit('newTab', { el: this, name: 'Control', priority: -90 }))
  }
  ...
  newControl(c) {
    ...
    if(!tab) {
      tab = { name: c.tab, el: this.new(FlexBalanced, { basis: 12 }, false) }
      this.right.appendChild(tab.el)
      this.newTab(tab)
    }
    ...
    if(c.el) {
      tab.el.appendChild(c.el)
    }
    else if(module = moduleAvailable(`./UI/${c.cat}`)) {
      this.skipCat.push(c.cat)
      tab.el.new(require(module), { command: c, parent: this })
    }
    else {
      var p = tab.el.new('p')
      var a = p.new('a')
      ...
      if(c.arg === null) {
        ...
        a.new('b', { textContent: c.gcode })
        a.newText(' ' + c.name)
        a.new('br')
        a.new('small', { textContent: c.arg || ' ' })
      }
