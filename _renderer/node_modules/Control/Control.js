...
module.exports = class Control extends TabComponent {
  initCallback() {
    ...
    defineAppEvent('newControl', 'Add item to Control', 'Control', "{ tab: '', cat: '', gcode: '', name: '' or elem: <HTMLElement>, tab: '' }")
    self.on('newControl', e => this.newControl(e.d))
    
    var div = this.newElem('div', true, { className: 'column-system' })
    this.left = div.newElem('left')
    this.right = div.newElem('right')
  ...
  readyCallback() {
    AppEvent('newTab', { elem: this, name: 'Control', priority: -90 })
  }
  ...
  newControl(c) {
    ...
    if(!tab) {
      tab = { name: c.tab, elem: this.newElem(FlexBalanced, false, { basis: 12 }) }
    ...
    if(c.elem) {
      ...
      tab.elem.newElem(require(module), true, { command: c, parent: this })
    }
    else {
      var p = tab.elem.newElem('p')
      var a = p.newElem('a')
      ...
      if(c.arg === null) {
        ...
        a.newElem('b', true, { textContent: c.gcode })
        a.newText(' ' + c.name)
        a.newElem('br')
        a.newElem('small', true, { textContent: c.arg || ' ' })
      }
