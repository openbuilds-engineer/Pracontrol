...
module.exports = class Module extends WebComponentAbstract {
  initCallback() {
    ...
    this.loadUserModulesDir().then(res => {
      ...
      this.el = this.newElem('div', true, { className: 'column-system' })
      this.left = this.el.newElem('div')
      this.right = this.el.newElem('div')
      
      this.left.newElem('h1').textContent = 'Module list'
      this.left.newElem('p').innerHTML = 'Edit module list as you want.<br>Be careful load only trusted modules!'
      this.textarea = this.left.newElem('p').newElem('textarea', true, { value: this.ser.conf })
      this.textarea.on('input', e => this.ser.conf = this.textarea.value)
      this.left.newElem('p').innerHTML = `<a onclick="location.reload()" class="button">Reload to take effect</a>`
    
      var exampleFile = require.resolve('Example')
      this.right.newElem('h1').textContent = 'Modules development'
      this.right.newElem('div').innerHTML = `
<ol>
  ...
  loadModules() {
    this.ser.conf.split('\n').forEach(s => s && setImmediate(() => App.newElem(require(s), false) ))
  }
