...
module.exports = class Slicer25D_Slicer extends WebComponentAbstract {
  initCallback() {
    ...
    this.left = div.newElem('div', true, { textContent: 'Drop image into app.' })
    this.right = div.newElem('div')
  ...
  slice(p) {
    ...
    this.getImagePath(p).then(p => {
      var img = this.newElem('img', false, { src: p })
  ...
  sliceDone() {
    if(!this.path.length) {
      ...
      var a = this.left.newElem('a', true, { textContent: 'Reslice', className: 'button' })
  ...
  gCodeDone(gCode) {
    ...
    var a = this.left.newElem('a', true, { textContent: 'Print', className: 'button' })
