module.exports = class Files extends WebComponentAbstract {
  init() {
    defineEvent('openFile', 'Window emits openFile when file is dragged into app', 'Core', 'e.dataTransfer.files')
    self.on('drop', e => this.fileDrop(e))
    self.on('dragover', e => this.dragHover(e))
    self.on('dragleave', e => this.dragEnd(e))
    
    defineShortcut('⌘O', 'Open file', 'General')
    self.on('keydown', e => (e.metaKey || e.ctrlKey) && e.key == 'o' && this.openDialog())
    
    defineEvent('openFileDialog', 'Opens file dialog', 'Core')
    self.on('openFileDialog', e => this.openDialog())
  }
