module.exports = class SerialBuffer extends WebComponentAbstract {
  init() {
    this.new(require('./ResetCheck'))
    this.new(require('./Busy'))
    
    this.buffer = []
    self.on('deviceReady', e => this.deviceReady = true)
    self.on('deviceBusy', e => this.deviceReady = false)
    
    defineEvent('serialWrite', 'Send data to serial buffer', 'Serial buffer', `{ data: '' or [], prepend: false }`)
    self.on('serialWrite', e => this.send(e.detail.data, e.detail.prepend))
    
    defineEvent('serialClean', 'Clean serial buffer', 'Serial buffer')
    self.on('serialClean', e => { this.buffer = []; this.bufferChange(); })
    
    // sending buffer
    self.on('deviceReady', e => setImmediate(e => this.sendBuffer()))
    
     defineEvent('bufferChange', 'Buffer was modified', 'Serial buffer', '[]')
    setImmediate(() => this.bufferChange())
  }
  
  send(data, prepend) {
    if(!Array.isArray(data)) data = String(data).split('\n')
    
    if(prepend) data.reverse()
    
    data.forEach(i => {
      if(!i) return
      prepend ? this.buffer.unshift(i + '\n') : this.buffer.push(i + '\n')
    })
    
    this.bufferChange()
    this.sendBuffer()
  }
  
  sendBuffer() {
    if(!this.deviceReady) return
    
    var data = this.buffer.shift()
    if(!data) return
    
    self.emit('serialDirectWrite', { data })
    this.bufferChange()
  }
  
  bufferChange() {
    self.emit('bufferChange', this.buffer)
  }
}
