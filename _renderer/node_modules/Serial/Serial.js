...
module.exports = class Serial extends WebComponentAbstract {
  init() {
    ...
    this.serial = null
    this.buffer = this.new(require('./SerialBuffer'), { serial: this }, false)
    this.status = this.new(require('./SerialStatus'), { serial: this }, false)
    
    defineEvent('refreshPorts', 'Reload port list', 'Serial')
    defineEvent('refreshPortsDone', 'Port are reloaded, listen only', 'Serial')
    defineEvent('newPort', 'New port found, listen only', 'Serial', '{ port: <port> }')
  ...
  connect(e) {
    ...
    if(!this.connectForm.speed.value) {
      self.emit('disconnected')
      throw new Error('No speed selected')
    }
    ...
    this.serial = new SerialPort(port, {
      ...
      baudrate: parseInt(this.connectForm.speed.value),
    })
    
    this.serial.on('open', () => self.emit('connected'))
    this.serial.on('close', () => self.emit('disconnected'))
    this.serial.on('error', e => { self.emit('disconnected'); throw new Error(e.message); })
