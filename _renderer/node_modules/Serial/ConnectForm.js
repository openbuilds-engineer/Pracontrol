...
module.exports = class ConnectForm extends WebComponentAbstract {
  init() {
    ...
    this.speed = this.new('input', true, { type: 'number', value: this.ser.speed || 115200 })
    
    this.submit = this.new('input', true, { type: 'button' })
    this.submit.on('click', e=> this.submitClick(e))
    
    var l = this.new('label')
    this.reconnect = l.new('input', true, { type: 'checkbox', checked: this.ser.reconnect })
  ...
  submitClick(e) {
    self.emit(this.isConnected ? 'disconnect' : 'connect')
  }
}
