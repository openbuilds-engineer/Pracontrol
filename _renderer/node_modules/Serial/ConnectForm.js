...
module.exports = class ConnectForm extends WebComponentAbstract {
  initCallback() {
    ...
    this.port = this.newElem('select')
    this.port.on('click', e => AppEvent('refreshPorts'))
    
    this.speed = this.newElem('input', true, { type: 'number', value: this.ser.speed || 115200 })
    
    this.submit = this.newElem('input', true, { type: 'button' })
    this.submit.on('click', e=> this.submitClick(e))
    
    var l = this.newElem('label')
    this.reconnect = l.newElem('input', true, { type: 'checkbox', checked: this.ser.reconnect })
  ...
  newPort(e) {
    var o = this.port.newElem('option')
    o.port = e.d.port
