...
module.exports = class SerialStatus extends WebComponentAbstract {
  init() {
    ...
    this.bufferStatus = this.new('buffer-status', false)
    this.bufferStatus.renderCss(styleBuffer)
    this.bufferStatus.new('a', true, { textContent: 'Clean buffer' }).on('click', e => self.emit('serialClean'))
    ...
    this.busyStatus = this.new('status', true, { textContent: '●' })
    self.on('deviceOk', e => this.busyStatus.classList.add('enabled'))
    self.on('deviceNotOk', e => this.busyStatus.classList.remove('enabled'))
  }
  
  readyCallback() {
    self.emit('newPanel', { elem: this, position: 'bottom' })
    self.emit('newPanel', { elem: this.bufferStatus, position: 'bottom' })
  }
}
