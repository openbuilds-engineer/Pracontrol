...
module.exports = class Slicer25D_Slicer extends WebComponentAbstract {
  init() {
    ...
    this.left = div.new('div', true, { textContent: 'Drop image into app.' })
    ...
    this.slider = this.new('input', true, opt)
  ...
  slice(p) {
    ...
    this.getImagePath(p).then(p => {
      var img = this.new('img', false, { src: p })
  ...
  sliceDone() {
    if(!this.path.length) {
      ...
      var a = this.left.new('a', true, { textContent: 'Reslice', className: 'button' })
  ...
  gCodeDone(gCode) {
    ...
    var a = this.left.new('a', true, { textContent: 'Print', className: 'button' })
