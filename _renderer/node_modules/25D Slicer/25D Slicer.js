...
module.exports = class Slicer25D extends WebComponentAbstract {
  init() {
    ...
    this.slicer = div.new(require('./lib/slicer.js'), true, { parent: this })
    this.slicer.style.flexGrow = 3
    
    this.recent = this.left.new('p').new('select')
    this.recent.new('option', true, { textContent: 'Load recent files' })
    ...
    l.new('br')
    this.fidelity = l.new('input', true, opt)
    ...
    l.new('br')
    this.pass = l.new('input', true, opt)
    ...
    var l = this.left.new('p').new('label', true, { textContent: 'Controling gCode' })
    this.powerCmd = l.new('textarea')
    ...
    setImmediate(() => self.emit('newTab', { elem: this, name: "2.5D" }))
  }
  ...
  newRecentOption(p, selected) {
    ...
    var textContent = path.basename(path.dirname(p)) + '/' + path.basename(p)
    var opt = this.recent.new('option', { path: p, textContent, selected }, false)
