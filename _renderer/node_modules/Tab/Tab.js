...
module.exports = class Tab extends TabComponent {
  init() {
    ...
    this.initTab()
    defineEvent('newTab', 'Create a new tab', 'Core', `{ el: <Element>, name: '', focus: bool, priority: 0 }`)
    self.on('newTab', e => { this.newTab(e.detail); self.emit('newPanel', { el: e.detail.el }); })
    self.on('focusTab', e => this.focusTab(e.detail))
    
    defineShortcut('⌘1‑9', 'Go to tab 1‑9', 'General')
    self.on('keydown', e => (e.metaKey || e.ctrlKey) && this.tabNumberShortcut(e))
    
    setImmediate(() => self.emit('newPanel', { el: this, position: 'top' }))
  }
