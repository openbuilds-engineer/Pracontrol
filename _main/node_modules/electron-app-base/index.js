...
module.exports = class {
  constructor(opt) {
    if(opt.constructor == String) opt = { index: opt }
    
    this.index = opt.index
    this.rem = opt.rem || 17
    
    app.on('window-all-closed', e => app.quit())
    app.on('ready', e => require('./menu'))
    app.on('ready', e => this.window())
  }
  
  window() {
    this.win = new electron.BrowserWindow({
      title: app.getName(),
      width: 41 * this.rem, height: 30 * this.rem,
      minWidth: 30 * this.rem, minHeight: 30 * this.rem,
    ...
    this.win.loadURL('file://' + this.index)
