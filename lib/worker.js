...
var gCode = function(powerGCode, fidelity, passes) {
  ...
  this.gCodePush = (x, y, s, p) => this.gCode.push.apply(this.gCode, eval('`' + powerGCode + '`').split("\n"))
  ...
  for(var pass = 0; pass < passes; pass++) {
    pathArr.forEach(path => {
      path.forEach((point, i) => {
        ...
        if(i == 0) this.gCodePush(point.x, point.y, 0, pass)
        
        this.gCodePush(point.x, point.y, point.s, pass)
        
        if(i == path.length - 1) this.gCodePush(point.x, point.y, 0, pass)
      })
    })
  }
